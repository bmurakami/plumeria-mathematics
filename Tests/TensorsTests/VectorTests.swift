...
@Test func VectorDenseReference_initComplex() {
    ...
    let v = VectorDenseReference([s, t])
    #expect(v.size == a.count)
    #expect(v[0].re == a[0])
    #expect(v[0].im == a[1])
...
@Test func VectorDenseReference_plus() {
    let u = VectorDenseReference(a)
    let v = VectorDenseReference(b)
...
@Test func DenseVector_toArray() throws {
    let v1 = VectorDenseReference([1.2, 3.4, 5.6])
